...
export const ReportResultsByVehicleType = (props: Props) => {
  const [results, setResults] = useState<ResultsType>();
  const [loading, setLoading] = useState<boolean>(false);

  const { type, vehicleType } = props;
  useEffect(() => {
    const fetchReport = async (type: string) => {
      setLoading(true);
      try {
        const res = await axios.get(
          `${url}/accidents/reports/vehicleTypes?vehicleType=${vehicleType}`
        );
        setResults(res.data);
        setLoading(false);
      } catch (error) {
  ...
  const total = Number(serious + slight + noInjury + fatal)
